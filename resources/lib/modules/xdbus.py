...
class xdbus:
    ...
    def __init__(self, oeMain):
        try:
            ...
            self.oe.dbg_log('xdbus::__init__', 'ERROR: (' + repr(e)
                            + ')', 4)
    ...
    def start_service(self):
        try:
            ...
            self.oe.dbg_log('xdbus::start_service', 'ERROR: (' + repr(e)
                            + ')', 4)                  

    def stop_service(self):
        try:

            self.oe.dbg_log('xdbus::stop_service', 'enter_function'
                            , 0)
            ...
            self.oe.dbg_log('xdbus::stop_service', 'exit_function',
                            0)
        except Exception, e:

            self.oe.dbg_log('xdbus::stop_service', 'ERROR: ('
                            + repr(e) + ')')
    ...
    def restart(self):
        try:
            ...
            self.oe.dbg_log('xdbus::restart', 'enter_function'
                            , 0)

            self.stop_service()
            self.start_service()

            self.oe.dbg_log('xdbus::restart', 'exit_function',
                            0)
        except Exception, e:

            self.oe.dbg_log('xdbus::restart', 'ERROR: ('
                            + repr(e) + ')')
...
class dbusMonitor(threading.Thread):
    ...
    def __init__(self, oeMain):
        try:
            ...
            oeMain.dbg_log('xdbus::dbusMonitor::__init__',
                           'enter_function', 0)
            ...
            self.oe.dbg_log('xdbus::dbusMonitor::__init__',
                            'exit_function', 0)
        except Exception, e:
    ...
    def run(self):
        try:
            ...
            self.oe.dbg_log('xdbus::dbusMonitor::run',
                            'enter_function', 0)
            ...
            self.oe.dbg_log('xdbus::dbusMonitor::run', 'ERROR: ('
                            + repr(e) + ')', 4)
    
    def stop(self):
        try:

            self.oe.dbg_log('xdbus::dbusMonitor::stop_service',
                            'enter_function', 0)
