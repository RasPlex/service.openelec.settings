    ...
    def adapter_powered(self, adapter, state=1):
        try:
            ...
            if int(self.adapter_info(self.dbusBluezAdapter, 'Powered')) != state:
                ...
                adapter_interface.Set('org.bluez.Adapter1', 'Alias', dbus.String(os.environ.get('HOSTNAME', 'RasPlex')))
            ...
            try:
                ...
                if 'Status' in interface:
                    if interface['Status'] == 'active':
                        ...
                        self.parent.download.create('RasPlex Bluetooth Filetransfer', '%s: %s' % (self.oe._(32181).encode('utf-8'),
                        ...
                        if interface['Status'] == 'complete':
                            ...
                            answer = xbmcDialog.yesno('RasPlex Bluetooth Filetransfer', self.oe._(32383).encode('utf-8'))
                            if answer == 1:
...
class bluetoothAgent(dbus.service.Object):
    ...
    def AuthorizeService(self, device, uuid):
        try:
            ...
            xbmcDialog = xbmcgui.Dialog()
            answer = xbmcDialog.yesno('RasPlex Bluetooth', 'Authorize service', 'Authorize service %s?' % (uuid))
    ...
    def RequestConfirmation(self, device, passkey):
        try:
            ...
            xbmc.executebuiltin('Dialog.Close(busydialog)')
            xbmcDialog = xbmcgui.Dialog()
            answer = xbmcDialog.yesno('RasPlex Bluetooth', 'Request confirmation', 'Confirm passkey %06u' % (passkey))
    ...
    def RequestAuthorization(self, device):
        try:
            ...
            xbmc.executebuiltin('Dialog.Close(busydialog)')
            xbmcDialog = xbmcgui.Dialog()
            answer = xbmcDialog.yesno('RasPlex Bluetooth', 'Request authorization', 'Accept pairing?')
...
class obexAgent(dbus.service.Object):
    ...
    def AuthorizePush(self, path):
        try:
            ...
            xbmc.executebuiltin('Dialog.Close(busydialog)')
            xbmcDialog = xbmcgui.Dialog()
            answer = xbmcDialog.yesno('RasPlex Bluetooth', self.oe._(32381), properties['Name'])
